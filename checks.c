...
static inline int check_username(str *_username, struct sip_uri *_uri)
{
	...
	{
		// if (authjwt_username_format == AUTHEPH_USERNAME_NON_IETF)
		// {
		// 	uname.s = _username->s;
		// 	uname.len = pos;
		// }
...
int authjwt_check_from0(struct sip_msg *_m)
{
	str username = {0, 0};

	if (eph_auth_api.pre_auth == NULL)
	{
		LM_ERR("authjwt_check_from() with no username parameter "
	...
	{
		LM_ERR("call authjwt_(check|proxy|www) before calling "
...
int authjwt_check_from1(struct sip_msg *_m, char *_username)
{
	str susername;
...
int authjwt_check_to0(struct sip_msg *_m)
{
	str username = {0, 0};

	if (eph_auth_api.pre_auth == NULL)
	{
		LM_ERR("authjwt_check_to() with no username parameter "
			"cannot be used without the auth module\n");
		return CHECK_ERROR;
	}
...
int authjwt_check_to1(struct sip_msg *_m, char *_username)
{
	str susername;
